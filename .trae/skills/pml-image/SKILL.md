...
```pml
;; 线性渐变 tile-mode：clamp / repeat / mirror / decal
(set-backend! "skia")
(canvas 720 160 :bg "#2d3436")

(define stops '((0.0 "#e17055") (1.0 "#74b9ff")))

(add (rect 20  20 160 120 :fill-gradient (linear-gradient stops :x1 0 :y1 0 :x2 0.25 :y2 0 :tile-mode "clamp")))
(add (rect 200 20 160 120 :fill-gradient (linear-gradient stops :x1 0 :y1 0 :x2 0.25 :y2 0 :tile-mode "repeat")))
(add (rect 380 20 160 120 :fill-gradient (linear-gradient stops :x1 0 :y1 0 :x2 0.25 :y2 0 :tile-mode "mirror")))
(add (rect 560 20 140 120 :fill-gradient (linear-gradient stops :x1 0 :y1 0 :x2 0.75 :y2 0 :tile-mode "decal")))

(render "gradient_tile_modes.png")
```

```pml
;; 径向 / sweep 渐变
(set-backend! "skia")
(canvas 480 220 :bg "#2d3436")

(add (rect 20  20 200 180
      :fill-gradient (radial-gradient '((0.0 "#00b894") (1.0 "#0984e3"))
                      :cx 0.5 :cy 0.5 :r 0.35 :tile-mode "decal")))

(add (rect 260 20 200 180
      :fill-gradient (sweep-gradient '((0.0 "#fdcb6e") (0.5 "#d63031") (1.0 "#fdcb6e"))
                      :cx 0.5 :cy 0.5 :start-angle 0 :end-angle 180 :tile-mode "repeat")))

(render "radial_sweep_gradients.png")
```
...
### `with-clip`
**签名**: `(with-clip graphic-object path)`

将 graphic-object 的渲染裁剪到 path 区域内，path 为任意 GraphicObject（形状、路径等）。

```pml
;; 将圆裁剪到矩形区域内
(add (with-clip (circle 100 100 80 :fill "#e94560")
      (rect 50 50 100 100)))
```
...
运行时检查着色器的 uniform 和 child shader 插槽：

| 函数 | 说明 |
|------|------|
| `(shader-uniforms handle)` | 返回 `((name type offset size) ...)` — 所有 uniform 的名称、类型字符串、字节偏移、字节大小 |
| `(shader-children handle)` | 返回 `((name type index) ...)` — 所有 child shader 插槽的名称、类型字符串、索引 |
| `(shader-validate handle uniform-data)` | 验证 uniform 数据大小是否匹配着色器声明，返回 `:ok` 或错误 |
| `(eval-shader handle x y)` | 在坐标 (x, y) 处采样着色器，返回 `(r g b a)` 颜色值（0.0–1.0）|
...
;; 调试：在特定点采样着色器
(define s (shader "half4 main(float2 xy) { return half4(xy.x/200, xy.y/200, 0, 1); }"))
(println (eval-shader s 100 50))  ;; → (0.5 0.25 0 1)
```

### 着色器组合

将 SkSL shader 与已有的 child shader 组合为 GPU 端管线：

| 函数 | 说明 |
|------|------|
| `(compose-with-child wrapper-sksl child-handle [:uniforms data])` | 组合单个 child shader，wrapper 中 `uniform shader child;` 接收 child |
| `(compose-with-children wrapper-sksl (list children...) [:uniforms data])` | 组合多个 child shader，`uniform shader src0; src1;` 引用 |

```pml
;; 对噪声着色器做色调映射
(define noise (noise-fractal :seed 42 :octaves 4))
...
;; 双 child：两个噪声之间做 mask
(define a (noise-fractal :seed 1))
(define b (noise-fractal :seed 2))
(define masked (compose-with-children "
  uniform shader src0;
  uniform shader src1;
  half4 main(float2 xy) {
    half4 mask = half4(xy.x/400, 0, 0, 1);
    return mix(src0.eval(xy), src1.eval(xy), mask);
  }
" (list a b)))
```
...
从外部图片文件直接创建 shader handle：

**签名**: `(image-shader "path.png")` → handle

```pml
(define tex (image-shader "texture.png"))
(add (apply-shader! (rect 0 0 200 200) tex))
```

### sksl-type-size

返回 SkSL 类型的字节大小，用于手动构造 uniform 数据：

**签名**: `(sksl-type-size "float2")` → `8`

### canvas->shader

将当前 canvas 的渲染结果直接转为 shader handle，用于后处理或作为 compose child：

**签名**: `(canvas->shader)` → handle

```pml
(canvas 400 400 :bg "#1a1a2e")
(add (rect 0 0 400 400 :fill (linear-gradient '((0.0 "#e17055") (1.0 "#74b9ff")))))
(define cs (canvas->shader))

(canvas 400 400 :bg "#1a1a2e")
(define processed (compose-with-child "
  uniform shader child;
  half4 main(float2 xy) {
    half4 c = child.eval(xy);
    return half4(c.b, c.g, c.r, 1);
  }
" cs))
(add (apply-shader! (rect 0 0 400 400) processed))
(render "post_processed.png")
```
...
### `perturb-polygon`
**签名**: `(perturb-polygon points [:edge-noise n] [:edge-subdiv n] [:corner-radius n] [:seed n] [:fill color] [:stroke color] [:stroke-width w] [:opacity o])`

对多边形边缘添加随机扰动，生成有机形状。

**返回值分两种情况：**

- **提供了样式参数**（`:fill`、`:stroke`、`:stroke-width`、`:opacity` 任一）时，直接返回 `GraphicObject`，可立即 `(add ...)`。
- **没有提供样式参数**时，返回扰动后的点列表 `((x1 y1) (x2 y2) ...)`，可以继续传给 `(polygon ...)` 做进一步处理，也可以链式调用多次 `perturb-polygon`。
...
;; 返回点列表，再交给 polygon
(define pts (perturb-polygon '((0 0) (200 0) (200 200) (0 200))
              :edge-noise 8.0 :edge-subdiv 3 :seed 1))
(add (polygon pts :fill "#3498db" :stroke "#2980b9" :stroke-width 2))
```
...
- `:corner-radius` — 圆角半径
- `:corner-mask` — 哪些角圆角化
- `:fill` / `:stroke` / `:stroke-width` / `:opacity` — 样式传透
...
**签名**: `(uniform-animate shader-handle uniform-name from to duration [:ease 'linear])`

对着色器的 float uniform 做基于时间线的动画，自动随时间插值，无需动画循环。

- `shader-handle` — 已有着色器句柄
- `uniform-name` — uniform 名称字符串
- `from` / `to` — 起止浮点值
- `duration` — 时长（秒）
- `:ease` — 缓动函数名（默认 `"linear"`）
...
(uniform-animate s "u_seed" 0.0 100.0 3.0)
```
...
| `(bit-xor a b)` | 按位异或 |
| `(bit-not a)` | 按位非 |
| `(bit-shift a n)` | 左移（n>0）或右移（n<0） |

### `dotimes`（特殊形式）

**签名**: `(dotimes (i n) body...)` — 迭代 i 从 0 到 n-1，每次执行 body。

```pml
(dotimes (i 5)
  (add (circle (* i 40) 100 15 :fill "#e94560")))
```

### `for`（特殊形式）

**签名**: `(for (i start end [step]) body...)` — 数值 for 循环，start 包含，end 排除，step 可省略（默认 1）。

```pml
(for (i 0 100 10)
  (add (circle i 100 5 :fill "#74b9ff")))
```
...
| `(third xs)` | 列表的第三个元素 |
| `(rest xs)` | 列表剩余元素 |
| `(nth xs index)` | 按索引访问元素 |
| `(null x)` | 判断是否为 nil |
| `(atom x)` | 判断是否为原子（非 cons） |
| `(consp x)` | 判断是否为 cons 对 |
| `(listp x)` | 判断是否为列表 |
| `(last xs)` | 最后一个 cons 对 |
| `(butlast xs)` | 除最后一个外的所有元素 |

### 序列操作

| 函数 | 说明 |
|------|------|
| `(find item seq)` | 在序列中查找元素，返回元素或 nil |
| `(position item seq)` | 返回元素索引或 nil |
| `(count item seq)` | 统计出现次数 |
| `(remove item seq)` | 移除所有匹配元素 |
...
| 函数 | 说明 |
|------|------|
| `(name sym-or-val)` | 返回符号或值的名称字符串 |
| `(find category key)` | 按类别查找已注册项 |
| `(replace category key new-value)` | 替换已注册项 |
| `(remove category key)` | 移除已注册项 |
| `(set-property target key value)` | 设置对象的属性 |
