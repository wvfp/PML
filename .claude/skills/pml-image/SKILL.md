...
注意：此函数每次创建一个只带 4 字节的新 shader。多 uniform 场景请使用 `make-uniforms` + `apply-uniforms`。
...
#### `shader-uniforms`
**签名**: `(shader-uniforms shader-handle)`

查看编译后着色器的 uniform 声明列表。每条包含：名称、类型、字节偏移、字节大小。

```pml
(shader-uniforms my-shader)
;; → (("u_bl" "float" 0 4) ("u_br" "float" 4 4) ("u_type" "float" 8 4))
```

#### `shader-validate`
**签名**: `(shader-validate shader-handle uniform-data)`

预检查 uniform 数据是否与着色器声明兼容，返回 `:ok` 或错误信息。

```pml
(shader-validate my-shader (make-uniforms 0.5 1.0))
;; → :ok
```

#### `compose-with-child`
**签名**: `(compose-with-child wrapper-sksl child-handle)`

将一个现有 shader 作为 `src` child 组合到新的 SkSL wrapper 中。wrapper 必须声明 `uniform shader src;`。
零像素落地 — 直接在 GPU 端组合管线。

```pml
(define noise (noise-fractal :seed 42))
(define quantized (compose-with-child "
  uniform shader src;
  half4 main(float2 xy) {
    half4 n = src.eval(xy);
    float v = (n.r + n.g + n.b) * 0.333;
    return v < 0.5 ? half4(0.2,0.5,0.2,1) : half4(0.6,0.8,0.3,1);
  }
" noise))
```

#### `compose-with-children`
**签名**: `(compose-with-children wrapper-sksl children-list [:uniforms data])`

组合多个 child shader 到新的 SkSL wrapper 中。wrapper 使用 `child_0`、`child_1`… 声明插槽。可选 `:uniforms` 传 uniform 数据。
...
" (list noise-a noise-b) :uniforms (make-uniforms 0.5))
```

#### `eval-shader`
**签名**: `(eval-shader shader-handle x y)`

在像素坐标 (x, y) 处采样 shader 颜色，返回 `(r g b a)` 列表。用于调试"着色器编译成功但输出全黑"等问题。

```pml
(eval-shader my-shader 100 50)
;; → (0.45 0.23 0.89 1.0)
```
...
| 函数 | 说明 |
|------|------|
| `(->double x)` | 将数值显式转换为 Double（浮点数）类型 |
| `(->int x)` | 将数值显式转换为 Int（整数）类型 |
...
| 函数 | 说明 |
|------|------|
| `(div a b)` | 整数除法，为 `quotient` 别名 |
| `(quotient a b)` | 整数除法，返回整数商 |

### 迭代

| 形式 | 说明 |
|------|------|
| `(dotimes (var count) body...)` | 从 0 迭代到 count-1，执行 body |
| `(for (var start end [step]) body...)` | 数值 for 循环，start 包含，end 不包含，step 默认为 1 |
