...
export class PMLPreviewProvider implements vscode.CustomReadonlyEditorProvider<PMLDocument> {
   ...
   * import form. Also matches `(import "filename" as prefix)` and
   * `(import "filename" prefix)`.
   *
   * Does NOT handle:
   * - Dynamic imports generated by macros (impossible to detect statically).
   * - Imports inside string literals or comments (acceptable false positives
   *   — the worst that happens is an extra FileSystemWatcher).
   */
